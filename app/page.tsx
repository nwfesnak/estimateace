...
export default function Home() {
  ...
  // NEW: Convert to Invoice + open send popup
  const convertToInvoice = () => {
    setDocumentType('invoice');
    // Update number prefix from EST to INV
    if (invoiceNumber.startsWith('EST-')) {
      const newNum = invoiceNumber.replace('EST-', 'INV-');
    ...
    showMessage('Document converted to Invoice');
  };
  ...
  const sendViaEmail = () => {
    ...
    showMessage(`✅ Invoice sent via email to: ${selectedEmailsForSend.join(', ')}`);
    setIsSendModalOpen(false);
  };

  const sendViaText = () => {
    if (selectedPhonesForSend.length === 0) return showMessage("Please select at least one phone number");
    showMessage(`✅ Invoice sent via text to: ${selectedPhonesForSend.join(', ')}`);
    setIsSendModalOpen(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#f4f4f4] p-4 md:p-8">
      ...
      {/* Send Modal (used by both Send button and Convert to Invoice) */}
