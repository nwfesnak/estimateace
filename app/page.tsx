    ...
    <>
                      ...
                      {Object.entries(profile.paymentSettings || {}).map(([method, settings]: [string, any]) => (
                        <div key={method} className="flex items-center justify-between border rounded-2xl p-6">
                          <div className="flex items-center gap-4">
                            <span className="text-4xl">{method === 'stripe' ? '💳' : method === 'echeck' ? '🏦' : method === 'paypal' ? '💰' : method === 'venmo' ? '📱' : '🏦'}</span>
                            <div>
                              <div className="font-semibold capitalize text-lg">{method}</div>
                              <div className="text-sm text-gray-500">{settings.connected ? '✓ Connected' : 'Not connected'}</div>
                            </div>
                          </div>
                          <div className="flex items-center gap-6">
                            <label className="relative inline-flex items-center cursor-pointer">
                              <input type="checkbox" checked={settings.enabled} onChange={e => togglePaymentMethod(method, e.target.checked)} className="sr-only peer" />
                            ...
                            <Button onClick={() => linkPaymentAccount(method)} variant={settings.connected ? "outline" : "default"}>
      ...
      <Dialog open={isPaymentModalOpen} onOpenChange={setIsPaymentModalOpen}>
        <DialogContent className="max-w-md">
            ...
            <div className="space-y-3">
              {Object.keys(profile.paymentSettings || {}).map((method) => {
                ...
                return (
                  ...
                  >
                    <span className="text-3xl">{method === 'stripe' ? '💳' : method === 'echeck' ? '🏦' : method === 'paypal' ? '💰' : method === 'venmo' ? '📱' : '🏦'}</span>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={closePaymentModal} className="flex-1">Cancel</Button>
            <Button onClick={proceedWithPayment} disabled={!selectedPaymentMethod} className="flex-1 bg-[#10b981]">Continue to Pay</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
